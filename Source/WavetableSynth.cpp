...
std::vector<float>WavetableSynth::generateSineWaveTable()
{
	...
	for (auto i = 0; i < WAVETABLE_LENGTH; ++ i)
	{
		sineWaveTable[i] = std::sinf(2 * PI *
			static_cast<float>(i) / static_cast<float>(i)/(WAVETABLE_LENGTH));
	}
	return sineWaveTable;
}


void WavetableSynth::intializeOscillators()
{
	constexpr auto OSCILLATTORS_COUNT = 128;

	const auto sineWaveTable = generateSineWaveTable();


	oscillators.clear();
	for (auto i = 0; i < OSCILLATTORS_COUNT;++i)
	{
